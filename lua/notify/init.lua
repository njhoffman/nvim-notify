...
function notify.instance(user_config, inherit)
  ...
  local direction = instance_config.top_down() and stage_util.DIRECTION.TOP_DOWN or stage_util.DIRECTION.BOTTOM_UP

  animator_stages = type(animator_stages) == "string" and stages[animator_stages](direction) or animator_stages
  ...
  function instance.open(notif_id, opts)
    ...
    if not notif then
      vim.notify("Invalid notification id: " .. notif_id, vim.log.levels.WARN, { title = "nvim-notify" })
      return
    end
    local buf = opts.buffer or vim.api.nvim_create_buf(false, true)
    local notif_buf = NotificationBuf(buf, notif, vim.tbl_extend("keep", opts, { config = instance_config }))
    notif_buf:render()
    return {
      buffer = buf,
